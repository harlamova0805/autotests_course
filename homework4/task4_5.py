...
def josephus_task(num_people, kill_num):  # Решение по формуле (1 + (k(n-1, m) + m -1) mod n))
    survivor = [int(i) for i in range(1, num_people + 1)]  # Создаем список всех воинов
    while len(survivor) > 1:  # Пока выживших больше одного
        for j in range(0, kill_num - 1):  # В данном цикле нужно первые элементы, которые при счете пропускаются,
            # записать сразу в конец списка, чтобы потом продолжать счет по кругу
            survivor.append(survivor[j])
        del survivor[:kill_num]  # Удаляем первые элементы списка + выбывшего, первые ужа записаны в конец списка,
        # а значит при счете "по кругу" список не меняется, только удаляется первый выбывший, потом второй и т.д.
    survivor = survivor[0]  # Нужно вернуть не список, а элемент списка, а у нас список из 1 элемента
    return survivor
