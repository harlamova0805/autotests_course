...
def create_phone_number(num_tuple):
    num_str = "".join([str(i) for i in num_tuple])  # Объединяем элементы кортежа в строку без разделителей
    str_phone = f'({num_str[:3]}) {num_str[3:6]}-{num_str[6:]}'  # С помощью f-строк получаем номер телефона из срезов
    return str_phone
